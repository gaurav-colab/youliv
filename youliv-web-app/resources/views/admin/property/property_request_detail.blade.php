                                ...
                                <div class="col-md-6">
                                    <div class="pro-leased-det">
                                        ...
                                        <h6>Expiry of Lease: {{$propertyData->lease_expiry}}</h6>
										 ...
										 <figure >
                                       <a href="{{ url('')}}/public/{{$propertyData->lease_deed}}" target="blank"> <img src="{{ url('')}}/public/{{$propertyData->lease_deed}}" alt="address proof"></a>
                                        </figure>
                                        </div>
                                </div>
				...
				@else
					...
					@else
						...
						@if($propertyData->property_diff_address!="")
							Identity proof with same address : No<br>
							Address Proof :
					...
					<div class="col-md-12 ">
							...
							@if($propertyData->property_address_img!="")
							    <a href="{{ url('')}}/{{$propertyData->property_address_img}}" target="blank"><img src="{{url('')}}/{{ $propertyData->property_address_img }}" alt="proof front"></a>
							@endif
							</div>
					</div>					
					@endif
			...
			<div class="col-md-6">
			    <div class="card">
				    ...
				    <div class="card-body">
							...
							<?php 
							if($propertyData->address_city!="")
							{
								$city=App\City::where('id',$propertyData->address_city)->first();				
								$address_city=$city->name;
							}
							if($propertyData->address_sector!="")
							{
								$address_sector=App\Sector::where('id',$propertyData->address_sector)->first();
								$address_sector=$address_sector->slug;
							}						
							if($propertyData->address_state!="")
							{
								$address_state=App\State::where('id',$propertyData->address_state)->first();
								$address_state=$address_state->name;
							}	
							?>
							<p style="font-weight:500">House Address: {{$propertyData->address_house}} <br>
							Building Address: {{$propertyData->address_building}} <br>Street: {{$propertyData->address_street}} <br>Sector: {{$address_sector}} <br>City: {{$address_city}} <br>State: {{$address_state}} <br>Zipcode: {{$propertyData->zipcode}}</p>
		        ...
		        <div class="card">
				    ...
				    <div class="card-body amen-sec">
				        <div class="row neighbour1" >
						 <ul>
									<li>Area1 : {{$propertyData['area1']}} <br>Distance1 :{{$propertyData['distance1']}}</li>
									<li>Area2 : {{$propertyData['area2']}} <br>Distance2 :{{$propertyData['distance2']}}</li>
									<li>Area3 : {{$propertyData['area3']}} <br>Distance3 :{{$propertyData['distance3']}}</li>
									<li>Area4 : {{$propertyData['area4']}} <br>Distance4 :{{$propertyData['distance4']}}</li>
